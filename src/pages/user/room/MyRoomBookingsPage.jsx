...
import { ROOM_STATUS, BOOKING_STATUS } from '../../../utils/constants'; // using BOOKING_STATUS for consistency
...
export default function MyRoomBookingsPage() {
  const { user } = useAuth();
  const { getUserRoomBookings, cancelRoomBooking, submitRoomReview } = useRoomBooking();
  ...
  const handleCancel = async () => {
    if (cancelTarget) {
      ...
      try { 
        await cancelRoomBooking(cancelTarget, 'Dibatalkan oleh pengguna'); 
        toast.success('Peminjaman ruangan dibatalkan.'); 
      } catch (err) { 
  ...
  return (
    <div className="min-w-0">
      <PageHeader
        title="Kontrol Booking Ruangan Pribadi"
        subtitle="Lacak status pengajuan, check-in ruangan, dan batalkan permintaan yang masih menunggu persetujuan."
      />
        ...
        <div className="mobile-scroll-tabs">
          {TABS.map((tab) => (
            <button key={tab} onClick={() => setActiveTab(tab)}
              className={`interactive-pill flex-shrink-0 ${forcedTab === tab ? 'bg-blue-600 text-white border-blue-600 shadow-md' : 'bg-transparent text-[color:var(--color-text-muted)] border border-[color:var(--color-border)] hover:border-blue-500/50 hover:text-blue-500'}`}>
              {tab}
            </button>
          ))}
        </div>
      ...
      {filtered.length === 0 ? (
        ...
        <div className="space-y-4">
          {filtered.map((b) => (
            <Card key={b.id} id={`booking-${b.id}`} hover className="p-5 sm:p-6 transition-all duration-300 border-blue-500/10 hover:border-blue-500/30 shadow-sm hover:shadow-md">
              <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <div className="flex-1 min-w-0">
                  ...
                  <div className="mt-3 flex flex-wrap gap-x-4 gap-y-2 text-sm text-[color:var(--color-text-soft)]">
                    ...
                    <span>{b.jumlahPeserta} orang</span>
                  </div>
                  {b.roomName && <p className="mt-2 text-sm font-medium text-blue-600">{b.roomName}</p>}
                  {b.alasanPenolakan && <p className="mt-3 rounded-2xl bg-danger-light px-3 py-2 text-sm text-danger border border-danger/20">Alasan: {b.alasanPenolakan}</p>}
                </div>
      ...
      {/* Review Modal */}
      <Modal isOpen={!!reviewTarget} onClose={() => setReviewTarget(null)} title="Catatan Penggunaan Ruangan" size="md">
        <form onSubmit={handleReviewSubmit} className="space-y-4">
          <p className="text-sm text-[color:var(--color-text-soft)] mb-2">
            Peminjaman telah selesai. Tambahkan catatan jika terdapat kendala (seperti AC mati, proyektor bermasalah, kebersihan kurang, dll).
          </p>
          <FormInput
            id="reviewNotes"
